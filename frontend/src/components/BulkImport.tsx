...
export function BulkImport({ onImport, disabled = false, loading = false, progress = null }: Props) {
  ...
  return (
    <Card stack>
      ...
      >
        {loading ? `取り込み中... (${progress?.completed ?? 0}/${progress?.total ?? 0})` : "取り込み"}
      </button>
      {loading && progress && (
        <div className="bulk-progress" aria-live="polite">
          ...
          <div className="bulk-progress-track" role="progressbar" aria-valuemin={0} aria-valuemax={100} aria-valuenow={progressPercent}>
